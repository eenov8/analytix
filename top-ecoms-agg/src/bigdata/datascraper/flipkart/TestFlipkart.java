...
public class TestFlipkart {
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			if (driver.findElement(By.xpath("//div[@id='show-more-results']"))
					.isDisplayed()) {
				driver.findElement(By.xpath("//div[@id='show-more-results']"))
						.click();
			}
		...
		WebElement gotoTop = driver.findElement(By.className("fk-ui-goTop"));
		if (gotoTop != null)
			gotoTop.click();

		int pullThePlug = 0;
		// On the resulting page, load up all the elements with class as
		// pu-title
		List<WebElement> lstProducts = driver.findElements(By
				.className("pu-title"));
		for (WebElement webElement : lstProducts) {
			if (pullThePlug == 0) {
				// Create a JSON document in form of Java Map. This will be
				// persisted by the end of this loop.
				// HashMaps in Java will be persisted as embedded documents.
				// Array as embedded array and finally a key-value as a single
				// key-value.
				Map<String, Object> documentMap = new HashMap<String, Object>();
				documentMap.put("source", "flipkart");
				/*
				 * Map<String, Object> documentMapDetail = new HashMap<String,
				 * Object>(); documentMapDetail.put("records", "99");
				 * documentMapDetail.put("index", "vps_index1");
				 * documentMapDetail.put("active", "true");
				 * documentMap.put("detail", documentMapDetail);
				 */

				// Not interested in the title from this page, the product link
				// will be fetched and opened individually for complete details.
				...
				try {
					productTitle = driver_prod.findElement(By
							.xpath("//h1[@class='title']"));
					productSubTitle = driver_prod.findElement(By
							.className("subtitle"));
					productPrice = driver_prod.findElement(By
							.className("selling-price"));
					sellerName = driver_prod.findElement(By
							.className("seller-name"));
					sellerRating = driver_prod.findElement(By
							.className("rating-out-of-five"));
				...
				List<WebElement> productFeatures = driver_prod.findElements(By
						.xpath("//ul[@class='keyFeaturesList']/li"));
				...
				try {
					documentMap
							.put("primary_seller_name", sellerName.getText());
				} catch (NullPointerException ex) {
				...
				try {
					documentMap.put("product_main_features",
							productFeaturesArrayList.toArray());
				} catch (NullPointerException ex) {
					documentMap.put("product_main_features", "");
				}
				...
				// driver.findElement(By.className("selling-price "));
				// WebElement productSeller =
				// driver.findElement(By.className("seller-name"));

				// Since the sellers information is loaded only after clicking
				// the small hyperlink which says load N sellers
				// Clicking the above link is required to fetch the complete
				// details.
				try {
				...
				// Seller-wise price information is present in a data table with
				// class name "seller-table"
				// We will be iterating all the rows and corresponding columns
				// to fetch these details.
				try {
					WebElement htmltable = driver_prod
							.findElement(By
									.xpath("//div[contains(@class, 'seller-table')]/table[1]/tbody"));

					if (htmltable != null) {
						List<WebElement> rows = htmltable.findElements(By
								.tagName("tr"));

						// Map to create the embedded doc (seller name: selling
						// price)
						...
						for (int rnum = 0; rnum < rows.size(); rnum++) {
							...
							for (int cnum = 0; cnum < columns.size(); cnum++) {
								// This is the point where the data has to go
								// into a JSON object that can be later on
								// persisted into the MongoDB.
								if (cnum == 0)
									supplierName = columns.get(cnum).getText()
											.split("\\?")[0].split("\n")[0];
								if (cnum == 4)
									suppliedPrice = columns.get(cnum).getText()
											.split("\\?")[0].split("\n")[0];
